...
resource "aws_security_group_rule" "ecs_egress_https" {
  ...
  security_group_id = var.ecs_security_group_id
  description       = "Allow HTTPS for image pulls and AWS APIs"
}
...
# KMS Key for CloudWatch Logs encryption (CKV_AWS_158)
...
resource "aws_kms_key" "logs" {
  ...
  tags = merge(local.common_tags, {
    Name = "${var.project_name}-ecs-logs-kms"
  })
}
...
resource "aws_iam_role_policy" "ecs_task_minimal" {
  name = "${var.project_name}-ecs-task-minimal-policy"
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        {
          name      = "DB_HOST"
          valueFrom = "${var.secrets_manager_secret_arn}:host::"
        },
        {
          name      = "DB_USER"
          valueFrom = "${var.secrets_manager_secret_arn}:username::"
        },
        {
          name      = "DB_PASSWORD"
          valueFrom = "${var.secrets_manager_secret_arn}:password::"
        },
        {
          name      = "DB_NAME"
          valueFrom = "${var.secrets_manager_secret_arn}:dbname::"
        },
        {
          name      = "DB_PORT"
